...
const ClientA = () => {
    ...
    const getTdata = async () => {
        try {
            // Fliter the array 
            var clienta = data.filter(element => element.client_name == "[SAMPLE] Client A");//No Hardcoding
            console.log(clienta);

            var clta = data.filter(element => element.id == "28185505");//No Hardcoding
            console.log(clta);
                                            ...
                                            <tbody >
                                                {tdata.map(item => (
                                                    <tr className="">
                                                        ...
                                                        <td> {item.money_budget_spent > 1000 ? '$ '.concat(item.money_budget_spent.toFixed(2)) : (item.budget_spent.toFixed(2))} </td>
                                                        <td>{item.money_budget_left_percentage > 0 ? (item.money_budget_left_percentage.toPrecision(2).concat('%')) : (item.budget_left_percentage.toPrecision(2).concat('%'))}</td>
