...
const ClientB = () => {
   ...
   const getTdata = async () => {
      try {
         ...
         // applying filter
         var clienta = data.filter(element => element.client_id == "10586978");
         // console.log(clienta);
   ...
   return (
      ...
      <table id="clientB"
         className="table table-hover table-responsive-sm no-gutters "
         style={{ width: '100%' }}>
         <thead className="table-secondary text-dark ">
            <tr>

               <th style={{ width: '40%' }}><u>[SAMPLE] CLIENT B </u></th>
         ...
         <tbody >
            {tdata.map(item => (
               <tr className="">
                  ...
                  <td>{item.cost_budget > 0 ? '$ '.concat(item.cost_budget.toFixed(2)) : (item.budget.toFixed(2))}</td>
                  <td > {item.money_budget_spent <= 0 ? '$ '.concat(item.money_budget_spent.toFixed(2)) : (item.budget_spent.toFixed(2))} </td>
                  <td> {item.money_budget_left_percentage > 0 ? (item.money_budget_left_percentage.toFixed(0).concat('%')) : (item.budget_left_percentage.toFixed(0).concat('%'))} </td>
